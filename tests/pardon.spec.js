...
test('Pardon은 녹음 STT가 무의미하면 실시간 원문을 기준으로 복구해 보여준다', async ({ page }) => {
  ...
  await expect(page.locator('#recovered-transcript')).toContainText(liveTranscript);

  expect(errors).toEqual([]);
});

test('Pardon은 동일하게 깨진 두 STT도 기준 문장과 정적 변환에서 함께 바로잡는다', async ({ page }) => {
  const errors = [];
  page.on('pageerror', (error) => errors.push(error.message));
  page.on('console', (message) => {
    if (message.type() === 'error') errors.push(message.text());
  ...
  const brokenTranscript = '가장 맞는 카드 하나를 고르면 아래 결과가 바로 바힙니다';
  await installBrowserMocks(page, {
    liveTranscript: brokenTranscript,
    recordedTranscript: brokenTranscript,
    variants: []
  });
  await page.goto('/');
  ...
  await sttButton.click();
  await expect(page.locator('#recorded-transcript')).toContainText('바힙니다');
  await expect(page.locator('#recovered-transcript')).toContainText('바뀝니다');

  await page.getByRole('button', { name: '변환' }).click();
  await expect(page.locator('#variant-list .variant-card')).toHaveCount(3);
  await expect(page.locator('#variant-list .variant-card').first()).toContainText('바뀝니다');
