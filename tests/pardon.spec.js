...
test('Pardon은 녹음 → 정지 → STT → 변환 → 확정 흐름을 테스트 더블로 실행한다', async ({ page }) => {
  ...
  await expect(page.locator('.variant-card__label').nth(0)).toHaveText('제안 1 · 오인식 보정');
  await expect(page.locator('.variant-card__label').nth(1)).toHaveText('제안 2 · 문맥 교정');
  await expect(page.locator('.variant-card__label').nth(2)).toHaveText('제안 3 · 매끄러운 문장');
...
test('Pardon은 녹음 STT가 무의미하면 실시간 원문을 기준으로 복구해 보여준다', async ({ page }) => {
  ...
  await installBrowserMocks(page, {
    ...
    variants: [
      { id: 'possibility-1', label: '제안 1 · 오인식 보정', text: '보통 검색은 목록 보기에서 하고 이런 UI에서는 검색바가 추가되겠죠.' },
      { id: 'possibility-2', label: '제안 2 · 문맥 교정', text: '보통 검색은 목록 보기에서 하고 이런 UI에는 검색바를 추가하면 되겠죠.' },
      { id: 'possibility-3', label: '제안 3 · 매끄러운 문장', text: '이런 UI에서는 목록 보기 옆에 검색바를 추가하면 됩니다.' }
    ]
  });
  await page.goto('/');
