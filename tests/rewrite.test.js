...
test('buildRewriteVariants는 UI 설명 STT를 검색바/목록 보기 문맥으로 실제 교정한다', () => {
  ...
  assert.ok(texts.every((text) => !text.includes('검색바가 추가가')));
  assert.ok(!variants[1].text.includes('목록 보기에서 하잖아요. 목록 보기에서 보면'));
});
...
test('buildRewriteVariants는 GET/POST 누락 설명을 405 문맥으로 교정한다', () => {
  ...
  assert.ok(variants[2].text.includes('라우터'));
  assert.ok(variants.every((variant) => !variant.text.includes('겟 포스트')));
});

test('buildRewriteVariants는 짧은 안내문에서도 뒤쪽 교환 안내를 날리지 않는다', () => {
  const raw = '차시 시작하겠습니다 본 의약품은 의약품 제조 및 품질 관리 기준 법에 의한 품질 관리를 필요한 제품입니다 만약 구입시 변질 변패 또는 오손 된 제품이 발견 될 경우에는 구입처를 통하여 교환 부탁드립니다';
  const variants = buildRewriteVariants(raw);
...
test('compareTranscriptSources는 실시간을 기준으로 두고 녹음 STT는 보조 증거로만 다룬다', () => {
  ...
  assert.equal(result.chosenSource, 'live');
  assert.equal(result.recoveredText, live);
  assert.ok(result.recordedScore >= result.liveScore || result.liveScore >= result.recordedScore);
  assert.ok(result.summary.includes('보조 증거'));
});
