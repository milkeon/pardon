...
async function callOpenAIGPT(transcript) {
  const apiKey = process.env.OPENAI_API_KEY || '';
  if (!apiKey) throw new Error('서버에 OpenAI API Key 설정이 필요합니다. (.env 확인)');
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: '화자는 한국어와 영어를 수시로 혼용하여 사용하는 IT 엔지니어/개발자입니다. 브라우저 머신러닝의 한계로 인해, 영어를 강제로 억지 한글 발음으로 받아썼거나(예: "에이피아이", "커밋해줘", "도커") 발음이 심하게 꼬여 오인식되었을 확률이 매우 높습니다. 해당 발음이 본래 무엇을 의미하려 한 것인지 유연하게 유추하여, 영어(API, Git, commit, Docker, DB, PR, server 등)와 한글이 올바르게 혼용된 고도로 자연스러운 실무 개발자 문장 3가지 가능성을 엄격한 JSON 형태로 추정 반환하십시오. 키는 p1, p2, p3만 사용하고, 설명은 절대로 덧붙이지 마세요.'
        },
        {
          role: 'user',
    ...
    if (req.url === '/api/analyze' && req.method === 'POST') {
      ...
      req.on('end', async () => {
        try {
          ...
          const variants = await callOpenAIGPT(transcript);
          res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
